...
description: This document explains how to configure a remote Supabase instance for Miagent.io.
---

Miagent.io uses Supabase as a backend. This document explains how to configure a remote Supabase instance hosted at [supabase.com](https://supabase.com/).

## Creating a Remote Supabase Instance

Miagent.io comes with a CLI utility to create a backend instance on [supabase.com](https://supabase.com/):
...
If you already created a project on [supabase.com](https://supabase.com/), you can configure the Miagent.io frontend to use it.
...
Miagent.io requires that you set up an email provider on your Supabase instance. If you don't have one yet, please follow the instructions detailed in the [Email Provider Setup](#email-provider-setup) section.
...
In order to support [the invitations workflow](../users/user-management#adding-users) and the default email/password authentication (including password reset), Miagent.io needs to be able to send emails from your Supabase instance.
...
Miagent.io uses Supabase's authentication system. When a user logs in, Supabase redirects them to an authentication callback URL that is handled by the frontend.
...
If you host Miagent.io under the `https://example.com/atomic-crm/` URL, the callback URL should be `https://example.com/atomic-crm/auth-callback.html`.

## Customizing Email Templates

Miagent.io uses Supabase to send authentication-related emails (confirm signup, reset password, etc).

...
description: This document explains some of the architecture decisions made in the development of Miagent.io.
---

This document explains some of the architecture decisions made in the development of Miagent.io.
...
For the backend, Miagent.io leverages [Supabase](https://supabase.com/). Supabase provides a PostgreSQL database, a REST API, authentication, storage, and edge functions. In practice, this means that there is very little backend code to maintain (a few edge functions for user management and inbound email processing).

## Mutable Dependencies

The Miagent.io code primarily lives in the `src/components/atomic-crm` directory. With less than 15,000 lines of code, it is a small codebase that can be easily customized.

The Miagent.io code also contains some mutable dependencies, i.e. libraries that are directly integrated into the codebase to be modified if needed. These dependencies are located in the following directories:
...
Some pages in Miagent.io require data from multiple tables. To simplify the frontend code and reduce the HTTP overhead, Miagent.io uses database views to abstract the complexity of the queries.
...
User credentials are stored in Supabase's `auth.users` table. Supabase does not allow to add columns to this table. That's why additional user details are stored in a `sales` table created by Miagent.io. A database trigger is used to automatically sync the `sales` record when a user is created or updated (e.g. for the `first_name` and `last_name` fields).
...
To solve this problem, Miagent.io uses a `users` edge function in charge of:

- Verifying the current user's permissions
- Creating and updating users

Miagent.io does not support user deletion to avoid data losses. Yet, it is possible to disable a user's account (relying on Supabase's ban feature).

...
description: Miagent.io is designed to be easy to customize and extend by developers with basic React and SQL skills.
---


Miagent.io is designed to be easy to customize and extend by developers with basic React and SQL skills. This guide will help you get started with installing, configuring, and deploying Miagent.io.
...
1. Fork the Miagent.io repository on GitHub to your own account.
...
Miagent.io now runs at [http://localhost:5173/](http://localhost:5173/).
...
It's easier to develop on a non-empty application. Miagent.io provides a sample dataset you can import to get started. Click the import button and choose the sample data file at `test-data/contacts.csv`. This will add 500 contacts and 55 companies to your local database.
...
The entry point of the application is the `src/App.tsx` file. By default, this file simply renders the `<CRM>` component, which is the root component of Miagent.io.
...
Check the [Customizing Miagent.io](./developers/customizing) guide for an exhaustive list of configuration settings you can customize.

## Adding Custom Fields

You can extend the data model of Miagent.io by adding custom fields to the existing entities (Contacts, Companies, Deals, Notes, and Tasks).
...
2. Choose the Tables section in the Database management panel. You will see the list of tables used by Miagent.io.
...
Miagent.io uses views to simplify the queries. You need to update the `contacts_summary` view to include the new `referred_by` field.
...
:::tip
Miagent.io leverages two libraries that provided high-level building blocks: [Shadcn Admin Kit](https://marmelab.com/shadcn-admin-kit/) for the application logic, and [Shadcn/ui](https://ui.shadcn.com/) for the UI components. The code for these libraries is copied in the `src/components` folder. 
:::

Miagent.io uses TypeScript to ensure type safety, so you'll need to modify the `Contact` type to include the new `referred_by` field. Open the `src/components/atomic-crm/types.ts` file and add the `referred_by` field to the `Contact` interface.
...
It's time to deploy your customized version of Miagent.io to production. You use [Supabase.com](https://supabase.com) to host the backend, and GitHub Pages to host the frontend. Both services offer free plans that are sufficient for Miagent.io.

First, login or register to [Supabase.com](https://supabase.com).

Miagent.io comes with a CLI utility to create a backend instance on Supabase.com:
...
The frontend of the CRM is a Single-Page App that can be deployed to any CDN. Since you have forked the Miagent.io repository on GitHub, you can use GitHub Pages to host the frontend.
...
![Miagent.io in Production](../images/atomic-crm-demo.png)

:::tip
Supabase.com and GitHub Pages are just two of the many services you can use to host your Miagent.io. You can [self-host a supabase instace](https://supabase.com/docs/guides/self-hosting), and use any static server or CDN (e.g. CloudFlare, Netlify, Vercel, etc.) to host the frontend.
:::

## Next Steps

Explore the documentation to learn more about Miagent.io:

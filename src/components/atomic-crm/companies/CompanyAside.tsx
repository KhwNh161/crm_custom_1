  ...
  return (
    <div className="hidden sm:block w-[250px] min-w-[250px] space-y-4">
      <div className="flex flex-row space-x-1">
        {link === "edit" ? (
          <EditButton label="Chỉnh sửa công ty" />
        ) : (
          <ShowButton label="Xem thông tin công ty" />
        )}
      </div>
...
const CompanyInfo = ({ record }: { record: Company }) => {
  ...
  return (
    <AsideSection title="Thông tin công ty">
      {record.website && (
...
const ContextInfo = ({ record }: { record: Company }) => {
  ...
  return (
    <AsideSection title="Thông tin">
      {record.sector && (
        <span>
          Lĩnh vực: <TextField source="sector" />
        </span>
      )}
      {record.size && (
        <span>
          Quy mô: <SelectField source="size" choices={sizes} />
        </span>
      )}
      {record.revenue && (
        <span>
          Doanh thu: <TextField source="revenue" />
        </span>
      )}
      {record.tax_identifier && (
        <span>
          Mã số thuế: <TextField source="tax_identifier" />
        </span>
      )}
    </AsideSection>
  );
...
const AddressInfo = ({ record }: { record: Company }) => {
  ...
  return (
    <AsideSection title="Địa chỉ chính" noGap>

...
export const ContactMergeDialog = ({
  ...
  const handleMerge = async () => {
    if (!winnerId || !loserContact) {
      notify("Vui lòng chọn một liên hệ để gộp", { type: "warning" });
      return;
    }

    try {
      await mergeContacts(loserContact.id, winnerId);
      notify("Gộp liên hệ thành công", { type: "success" });
      redirect(`/contacts/${winnerId}/show`);
      onClose();
    } catch (error) {
      notify("Gộp liên hệ thất bại", { type: "error" });
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="md:min-w-lg max-w-2xl">
        <DialogHeader>
          <DialogTitle>Gộp Liên hệ</DialogTitle>
          <DialogDescription>
            Gộp liên hệ này với một liên hệ khác.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="p-4 bg-primary/5 rounded-lg border border-primary/20">
            ...
            <p className="font-medium text-sm mb-2">
              Liên hệ mục tiêu (sẽ được giữ lại)
            </p>
            <Form>
              <ReferenceInput
                source="winner_id"
          ...
          {winnerId && (
            <>
              <div className="space-y-2">
                <p className="font-medium text-sm">Các mục sẽ được gộp:</p>
              ...
              <Alert variant="destructive">
                ...
                <AlertTitle>Cảnh báo: Hành động phá hủy</AlertTitle>
                <AlertDescription>
                 Toàn bộ dữ liệu sẽ được chuyển đến liên hệ thứ hai.

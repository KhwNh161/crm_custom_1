    ...
    <div className="flex flex-col gap-2">
      ...
      <Card className="p-4 mb-2">
        <div className="flex flex-col gap-4">
          <TasksListEmpty />
          <TasksListFilter title="Quá hạn" filter={taskFilters.overdue} />
          <TasksListFilter title="Hôm nay" filter={taskFilters.today} />
          <TasksListFilter title="Ngày mai" filter={taskFilters.tomorrow} />
          {isBeforeFriday && (
            <TasksListFilter title="Tuần này" filter={taskFilters.thisWeek} />
          )}
          <TasksListFilter title="Để sau" filter={taskFilters.later} />
        </div>
      </Card>
    </div>
  );
